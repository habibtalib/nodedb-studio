...
- [ ] **CONN-05**: The active connection's `Capabilities` are derived from the server's real `capabilities()`/`limits()`, so the rail items, admin sub-tabs, and views gate on actual server capabilities
- [ ] **CONN-06**: The connection chip / identity (user, role, current database, databases list, server version) reflects the real session, not mock values
